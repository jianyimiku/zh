...
class Question(models.Model):
    ...
    tags = TaggableManager(help_text='多个标签', verbose_name='标签')
    has_answer = models.BooleanField(default=False, verbose_name='接受回答')
    votes = GenericRelation(Vote, verbose_name='内容')
    ...
    def get_upvoters(self):
        """赞同的用户"""
        # print([vote.user for vote in self.votes.filter(value=True).select_related('user').prefecth_related('vote')])
        return [vote.user for vote in self.votes.filter(value=True).select_related('user')]

    def get_downvoters(self):
        """反对的用户"""
        return [vote.user for vote in self.votes.filter(value=False).select_related('user')]
...
class Answer(models.Model):
    ...
    def get_downvoters(self):
        """反对的用户"""
        return [vote.user for vote in
                self.votes.filter(value=False).select_related('user')]
